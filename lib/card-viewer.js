...
function render(now) {
  ...
  if (events.click) {
    ...
    if (target.tagName === "FIGCAPTION") {
      ...
      if (hitIndex !== null && data[hitIndex].isUpload && data[hitIndex].onUploadClick) {
  ...
  if (newFocused == null) {
    ...
    const img1DSizeY =
      newWindowSizeY - windowPaddingTop - (data[newFocused].hasPrompt ? prompt1DSizeY : 0) - boxes1DGapY;
    const box1DMaxSizeX =
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      const imgSizeX =
        ...
        (isFocused ? 1 : 0.7);
      const sizeY = imgSizeX / d.ar + (d.hasPrompt && isFocused ? prompt1DSizeY : d.hasPrompt ? promptSizeY : 0);
